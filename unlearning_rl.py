...
from utils import  now_time, str2bool, get_loader, get_random_loader
...
def main(args):
    ...
    # 这个loader会倒转label
    forget_loader = get_random_loader('train', args.data_dir + args.forget_data, tokenizer, args.batch_size)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for retain_batch, forget_batch in zip(retain_loader, forget_loader):
            ...
            forget_gd = get_answer_loss(forget_batch, model)
            
            loss = retain_gd + forget_gd*args.weight
            ...
            if step % args.log_interval == 0 and step > 1000:
                ...
                print(now_time() + 'Step {:5d} Loss {:4.4f} retain gd {:4.4f} forget gd {:4.4f}'.format(step, loss, retain_gd, forget_gd))
